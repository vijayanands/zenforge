...
def engineering_metrics_dashboard():
    ...
    if st.button("Display Dashboard"):
        ...
        if 'eng_metrics_data' not in st.session_state:
            st.session_state.eng_metrics_data = None
        if 'show_eng_metrics' not in st.session_state:
            st.session_state.show_eng_metrics = False

        # Fetch metrics data
        with st.spinner('Loading engineering metrics... Please wait.'):
            metrics = calculate_metrics_from_timescale(start_date, end_date)
            
            # Update session state
            st.session_state.eng_metrics_data = metrics
            st.session_state.show_eng_metrics = True
            
            # Show success message and rerun
            st.success('Engineering metrics successfully loaded!')
            st.rerun()

    # Show dashboard only if data is loaded
    if st.session_state.get('show_eng_metrics', False):
        st.markdown("---")  # Add a separator
        
        # Use cached data
        metrics = st.session_state.eng_metrics_data
