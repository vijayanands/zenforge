...
def generate_sample_data():
    ...
    design_events = []
    design_phases = ['requirements', 'ux_design', 'architecture', 'database_design', 'api_design', 'security_review']
    ...
    for proj_id, details in projects.items():
        ...
        for phase in design_phases:
            ...
            design_events.append({
                ...
                'stakeholders': np.random.choice(['Product,Dev,QA', 'Dev,Arch', 'UX,Dev,Product', 'Dev,Security'], p=[0.4, 0.2, 0.2, 0.2]),
            ...
            for rev in range(num_revisions):
                design_events.append({
                    ...
                    'author': f"{phase.split('_')[0]}@example.com",
                    'jira': f"{proj_id}-{phase.upper()}-1",
                    'stakeholders': np.random.choice(['Product,Dev,QA', 'Dev,Arch', 'UX,Dev,Product', 'Dev,Security']),
    ...
    for proj_id, details in projects.items():
        ...
        for i in range(200):
            commit_date = start_date + timedelta(days=i//4)  # 4 commits per day on average
            ...
            commits.append({
                ...
                'commit_type': np.random.choice(['feature', 'bugfix', 'refactor', 'docs', 'test'], p=[0.4, 0.3, 0.15, 0.1, 0.05]),
        ...
        for i in range(50):
            ...
            for env in environments:
                ...
                build_metrics = {
                    ...
                    'failed_tests': np.random.randint(0, 10) if build_success else np.random.randint(10, 30),
    ...
    for proj_id in projects.keys():
        ...
        df_projects.loc[df_projects['project_id'] == proj_id, 'total_commits'] = len(proj_commits)
        df_projects.loc[df_projects['project_id'] == proj_id, 'avg_code_coverage'] = proj_commits['code_coverage'].mean()
        df_projects.loc[df_projects['project_id'] == proj_id, 'total_p0_bugs'] = len(proj_bugs)
