...
def fetch_commit_comments(owner, repo, start_date, end_date=None, token=None):
    """
    Fetch commit comments from a GitHub repository starting from a specific date.
    ...
    :return: List of commit comment data.
    """
    base_url = f'https://api.github.com/repos/{owner}/{repo}/comments'
...
def main():
    if len(sys.argv) < 4:
        print("Usage: python fetch_commit_comments.py <owner> <repo> <start_date> [end_date] [token]")
        print("Example: python fetch_commit_comments.py octocat Spoon-Knife 2021-01-01T00:00:00Z")
        sys.exit(1)
